...
import (
	"context"
	dpfm_api_input_reader "data-platform-api-planned_order-deletes-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-planned_order-deletes-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-planned_order-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) headerDelete(
	...
	header.PlannedOrder = input.Header.PlannedOrder
...
func (c *DPFMAPICaller) itemDelete(
	...
	for _, v := range input.Header.Item {
		...
		if !checkResult(res) {
			output.SQLUpdateResult = getBoolPtr(false)
			output.SQLUpdateError = "PlannedOrder Item Data cannot delete"
			return nil
		}
	}
	// itemがキャンセル取り消しされた場合、headerのキャンセルも取り消す
	if !*input.PlannedOrder.Item[0].IsMarkedForDeletion {
		header := c.HeaderRead(input, log)
